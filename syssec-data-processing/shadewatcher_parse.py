...
def parse(graph_paths, force_parse):
    def parse_graph(graph_path):
        ...
        encoding_parser.encode(
            edgefile_path=ENCODING_PATH + "/" + instance_name + "/edgefile_0.txt",
            nodefile_path=ENCODING_PATH + "/" + instance_name + "/nodefact.txt",
        ...
        subprocess.call(
            ["cp", "-R", ENCODING_PATH + "/" + instance_name, graph_store_dir]
        )

    with Pool(20) as pool:
        pool.map(parse_graph, graph_paths)
